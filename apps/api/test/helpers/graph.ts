...
export function isFullGraphResponse(body: unknown): body is FullGraphResponse {
  ...
  if (
    ...
    return false;
  if (!('rootNodeId' in data) || typeof data.rootNodeId !== 'string')
    return false;

  return true;
}

export function isDocumentSubgraphResponse(
  body: unknown,
): body is DocumentSubgraphResponse {
  if (typeof body !== 'object' || body === null) return false;
  if (!('success' in body) || body.success !== true) return false;
  if (!('data' in body) || typeof body.data !== 'object' || body.data === null)
    return false;
  ...
  if (
    !('directEdges' in data) ||
    !Array.isArray(data.directEdges) ||
    !data.directEdges.every(isGraphEdge)
  )
    return false;
  if (
    !('neighborNodes' in data) ||
    !Array.isArray(data.neighborNodes) ||
    !data.neighborNodes.every(isGraphNode)
  )
    return false;
...
export function isRebuildGraphResponse(
  body: unknown,
): body is RebuildGraphResponse {
  if (typeof body !== 'object' || body === null) return false;
  if (!('success' in body) || body.success !== true) return false;
