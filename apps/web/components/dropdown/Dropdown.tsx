...
export const DropdownMenuLabel = (props: DropdownMenuLabelProps) => (
  <DropdownMenuPrimitive.Label
    {...props}
    className={cn(
      'text-subtle px-2 pb-1 pt-1.5 text-xs font-semibold leading-none',
      props.className,
    )}
  />
);
    ...
    >
      ...
      {!checked && (
        <input
          aria-disabled={true}
          aria-label={
            typeof children === 'string' ? `Not active ${children}` : undefined
          }
          aria-readonly
          checked={false}
          type="checkbox"
      ...
      <DropdownMenuPrimitive.ItemIndicator asChild>
        <input
          ...
          aria-readonly
          aria-label={
            typeof children === 'string' ? `Active ${children}` : undefined
          }
          checked={true}
          type="checkbox"
  ...
  return (
    <ButtonOrLink
      ...
      className={cn(
        ...
        color === 'destructive'
          ? 'hover:bg-error hover:text-error text-error'
          : 'hover:bg-subtle',
        props.className,
      )}
    >
      <>
        {CustomStartIcon ||
          (StartIcon && (
            <Icon name={StartIcon} className="mr-1 h-4 w-4 shrink-0" />
          ))}
        <div
          className={cn(
            'w-full text-left text-sm font-medium leading-none',
            childrenClassName,
          )}
        >
          {children}
...
>(({ className = '', ...props }, forwardedRef) => {
  return (
    <DropdownMenuPrimitive.Separator
      className={cn('bg-emphasis -mx-1 my-1 h-px', className)}
      {...props}
      ref={forwardedRef}
    />
  );
